...
function requestLayer(layerUrl) {
    ...
    request.get(capabilityOpts, function(error, resp, body) {
        ...
        if (body.layers) {
            ...
            var ws = fs.createWriteStream(path.resolve(argv.f, body.id + '_' + body.name + '.json'));
...
function makeQuery(options, idList, start, end) {
	return function(callback) {
		request.get({
			...
			_wkid = _wkid || body.spatialReference.wkid || argv.srid;

			if (resp.statusCode === 200 && body.features) {
                var jstr = JSON.stringify(body.features.map(esriConverter.toGeoJson)),
                    lastComma = idList.objectIds.length <= end ? "" : ",";
                console.log('last: ' + lastComma);
                options.writer.write(jstr.substring(1, jstr.length-2) + lastComma);
                callback(null);
			} else {
