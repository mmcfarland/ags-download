    ...
    request.get(capabilityOpts, function(error, resp, body) {
        ...
        if (body.layers) {
            ...
            downloadLayer({uri: layerUrl, geometryType: body.geometryType}, function(geoJson) {
                if (argv.file) {
                    saveFile(geoJson, path.resolve(argv.f, body.id + '_' + body.name + '.json'));
	...
	return function(callback) {
		request.get({
			...
			if (resp.statusCode === 200 && body.features) {
				callback(null, body.features.map(esriConverter.toGeoJson));
			} else {
...
function saveFile(geoJson, filename) {
	fs.writeFile(filename.replace(' ', '_'), JSON.stringify(geoJson), function(err) {
		if (err) throw err;
	});
}
